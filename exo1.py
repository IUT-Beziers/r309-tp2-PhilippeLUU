...
class WishPacketTracer:
    ...
    def change_icon(self):
        if self.current_item:
            self.filename = filedialog.askopenfilename(filetypes=[("Image files", "*.png")])  # Boîte de dialogue pour sélectionner un fichier PNG
            if self.filename:
                self.photo=self.load_and_resize_image(self.filename,50,50)
                self.item_images[self.current_item] = self.photo
                self.canvas.itemconfig(self.current_item, image=self.photo)  # Changer l'image de l'élément actuellement sélectionné
